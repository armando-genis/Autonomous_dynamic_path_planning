...
void local_path_planning_node::sampleLayersAlongPath()
{
    if (waypoints_segmentation->empty())
    {
        std::cout << "Warning: waypoints segmentation is not available. Skipping polygon path creation." << std::endl;
        return;
    }

    if (rescaled_chunk_->data.empty())
    {
        std::cout << "Warning: combined map is not available. Skipping polygon path creation." << std::endl;
        return;
    }
    ...
    // DP structure to hold minimum cost paths
    std::vector<std::vector<double>> cumulative_costs;
    std::vector<std::vector<const Eigen::VectorXd *>> parents; // Keep track of parents for backtracking

    int num_layers_without_collision = 0;
    ...
    for (const auto &waypoint : *waypoints_segmentation)
    {
        ...
        std::vector<const Eigen::VectorXd *> layer_parents;
        ...
        for (double lateral_offset = -lateral_range; lateral_offset <= lateral_range; lateral_offset += lateral_spacing)
        {
            ...
            if (obstacle_distance > search_threshold)
            {
                ...
                num_layers_without_collision++;

                // Compute cost for this point based on previous layer's points
                double min_cost = std::numeric_limits<double>::max();
                const Eigen::VectorXd *best_parent = nullptr;

                if (!cumulative_costs.empty())
                {
                    for (size_t j = 0; j < cumulative_costs.back().size(); ++j)
                    {
                        double cost = cumulative_costs.back()[j] + computeCost(sample_point, layers_samples.back()[j]);
                        if (cost < min_cost)
                        {
                            min_cost = cost;
                            best_parent = &layers_samples.back()[j];
                        }
                    }
                }
                else
                {
                    min_cost = 0; // Initialize first layer with zero cost
                }

                layer_costs.push_back(min_cost);
                layer_parents.push_back(best_parent);
        ...
        // Store the feasible points and their costs for this layer
        layers_samples.push_back(layer_points);
        cumulative_costs.push_back(layer_costs);
        parents.push_back(layer_parents);
    }
    ...
    std::cout << green << "---> Number of feasible points: " << num_layers_without_collision << reset << std::endl;

    // Backtrack to find the optimal path
    std::vector<Eigen::VectorXd> optimal_path;
    const Eigen::VectorXd *best_point = nullptr;
    double min_total_cost = std::numeric_limits<double>::max();

    // Find the point with the lowest cumulative cost in the last layer
    for (size_t i = 0; i < cumulative_costs.back().size(); ++i)
    {
        if (cumulative_costs.back()[i] < min_total_cost)
        {
            min_total_cost = cumulative_costs.back()[i];
            best_point = &layers_samples.back()[i];
        }
    }

    // Backtrack to collect the optimal path
    for (int layer = cumulative_costs.size() - 1; layer >= 0; --layer)
    {
        if (best_point)
        {
            optimal_path.push_back(*best_point);
            best_point = parents[layer][best_point - &layers_samples[layer][0]];
        }
    }
