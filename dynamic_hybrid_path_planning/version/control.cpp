...
void dynamic_hybrid_path_planning_node::combineGrids(nav_msgs::msg::OccupancyGrid &grid, const nav_msgs::msg::OccupancyGrid &chunk)
{
    if (grid.info.resolution != chunk.info.resolution)
    {
        std::cout << red << "Error: resolution mismatch" << reset << std::endl;
        return;
    }
    std::cout << green << "Combining grids" << reset << std::endl;
}
...
void dynamic_hybrid_path_planning_node::global_gridMapdata(const nav_msgs::msg::OccupancyGrid::SharedPtr map)
{
    ...
    // if rescaled_chunk and grid exist combine them
    if (grid.data.size() > 0)
    {
        combineGrids(rescaled_chunk, grid);
    }
...
private:
    ...
    // functions to get the current robot state
    ...
    void combineGrids(nav_msgs::msg::OccupancyGrid &grid, const nav_msgs::msg::OccupancyGrid &chunk);
